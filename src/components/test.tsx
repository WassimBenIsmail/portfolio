...
const UseCasesSection = () => {
  ...
  const useCases = [
    {
      ...
      outcome:
        "Built internal AI tools → cut costs €100K/year & enabled autonomous R&D ops",
      context:
        "Bel Groupe needed to modernize their R&D operations and reduce operational costs while maintaining quality standards. The challenge was to implement AI-driven solutions that could automate repetitive tasks without disrupting existing workflows.",
      bulletPoints: [
    ...
    },
    {
      ...
      context:
        "Raisesense was an early-stage startup with a promising concept but needed to create a compelling MVP that would resonate with investors and demonstrate clear market potential.",
      bulletPoints: [
    ...
    },
    {
      ...
      outcome:
        "Scaled from zero to 60K+ users → monetized through partnerships",
      context:
        "Jorni needed to build a user base from scratch and establish sustainable revenue streams. The challenge was creating a platform that would attract users while developing profitable partnership opportunities.",
      bulletPoints: [
    ...
    },
    {
      ...
      outcome:
        "Improved energy dashboards → increased conversions and stickiness",
      context:
        "Metron's existing energy management dashboards were complex and difficult for users to navigate, resulting in low engagement and poor conversion rates. The goal was to redesign the user experience to be more intuitive and actionable.",
      bulletPoints: [
    ...
    },
    {
      ...
      context:
        "Schlumberger required a global enterprise solution to optimize operational workflows across multiple regions and business units. The challenge was creating a tool that could scale internationally while delivering significant cost savings.",
      bulletPoints: [
  ...
  const getTagIcon = (tag: string) => {
    if (tag.includes("Funds") || tag.includes("Cost"))
      return <DollarSign className="w-3 h-3" />;
    if (tag.includes("Growth") || tag.includes("Users"))
      return <Users className="w-3 h-3" />;
    if (tag.includes("Conversion") || tag.includes("Lift"))
      return <TrendingUp className="w-3 h-3" />;
    if (tag.includes("Global") || tag.includes("Enterprise"))
    ...
    <section id="use-cases" className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {selectedCase !== null && (
          ...
          >
            <div className="bg-white rounded-xl w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden relative">
              <button
                onClick={closeCase}
                className="absolute top-4 right-4 p-2 hover:bg-gray-100 rounded-full transition-colors z-20 bg-white shadow-md"
              >
                <X className="w-6 h-6" />
              </button>

              {/* Image Carousel Section */}
              <div className="bg-gray-50 relative flex-shrink-0 h-64 md:h-80">
                <button
                  onClick={prevImage}
              ...
              {/* Scrollable Content */}
              <div className="flex-1 overflow-y-auto p-6 md:p-8 pb-12">
                <div className="mb-8">
